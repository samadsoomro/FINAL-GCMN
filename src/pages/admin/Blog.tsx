...
import { Loader2, Plus, Pencil, Trash2, Eye, Pin, CheckCircle, XCircle, Image as ImageIcon, Save } from 'lucide-react';
// @ts-ignore
...
const AdminBlog: React.FC = () => {
    ...
    const handleTogglePin = async (id: string) => {
        try {
            const res = await fetch(`/api/admin/blog/${id}/pin`, { method: 'POST', credentials: 'include' });
            if (res.ok) {
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 p-6 shadow-lg border-primary/10">
                    <h3 className="text-xl font-semibold mb-6 flex items-center gap-2">
                        {isEditing ? <Pencil className="text-primary" size={20} /> : <Plus className="text-primary" size={20} />}
                ...
                <div className="lg:col-span-1 space-y-4">
                    ...
                    <div className="space-y-3 max-h-[800px] overflow-y-auto pr-2">
                        {posts.length === 0 ? (
                            ...
                            posts.map((post) => (
                                <Card key={post.id} className={`p-4 transition-all hover:shadow-md cursor-pointer border-l-4 ${post.status === 'published' ? 'border-l-emerald-500' : 'border-l-amber-500'} ${currentPost?.id === post.id ? 'bg-secondary' : ''}`} onClick={() => handleEdit(post)}>
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-semibold line-clamp-2 leading-tight">{post.title}</h4>
                                        {post.isPinned && <Pin size={14} className="text-primary shrink-0 fill-current" />}
                                    </div>
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
                                        <span className={`px-1.5 py-0.5 rounded-full ${post.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                                            {post.status}
                                        </span>
                                        <span>{new Date(post.createdAt).toLocaleDateString()}</span>
                                    </div>
                                    <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                                        <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleTogglePin(post.id)} title="Toggle Pin">
                                            <Pin size={14} className={post.isPinned ? "fill-primary text-primary" : "text-slate-400"} />
                                        </Button>
                                        <Button size="icon" variant="ghost" className="h-7 w-7 text-blue-600" onClick={() => window.open(`/blog/${post.slug}`, '_blank')} title="Preview">
                                            <Eye size={14} />
                                        </Button>
                                        <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => handleDelete(post.id)} title="Delete">
                                            <Trash2 size={14} />
                                        </Button>
                                    </div>
                                </Card>
