...
const renderMessageWithLinks = (text: string) => {
    ...
    const urlRegex = /(https?:\/\/[^\s]+)|(www\.[^\s]+)|([a-zA-Z0-9-]+\.(com|org|net|edu|gov|pk|io|co|us)[^\s]*)/g;
...
const NotificationsPage: React.FC = () => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0 }}
        >
            <div className="bg-gradient-to-br from-green-50 to-background py-16 text-center">
                <div className="container">
                    ...
                    >
                        Official announcements, news, and updates from Gov. College Formen, Nazimabad
                    </motion.p>
                </div>
            </div>
            ...
            <div className="container py-12 max-w-5xl">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        <Bell className="w-12 h-12 text-green-200 animate-bounce mb-4" />
                        ...
                        {notifications.map((notification, index) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    viewport={{ once: true }}
                                >
                                    <Card className={`overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300 ${notification.pin ? 'ring-2 ring-pakistan-green/20 bg-green-50/30' : 'bg-white'}`}>
                                        <div className={`flex ${isImageOnly ? 'flex-col' : 'flex-col md:flex-row'}`}>
                                            ...
                                            <div className={`p-6 md:p-8 flex-1 flex flex-col justify-center ${!isImageOnly && notification.image ? 'md:w-2/3' : 'w-full'}`}>
                                                <div className="flex flex-wrap items-center gap-2 mb-3">
                                                    ...
                                                    <span className="text-pakistan-green/80 text-xs font-bold flex items-center gap-1 uppercase tracking-wider">
                                                        <Clock size={12} /> {new Date(notification.createdAt).toLocaleDateString(undefined, { dateStyle: 'long' })}
                                                    </span>
                                                </div>
                    ...
                    <div className="text-center py-20 bg-neutral-50 rounded-3xl border-2 border-dashed border-neutral-200">
                        ...
                        <p className="text-neutral-500">There are no new announcements at this time.</p>
                    </div>
                )}
            </div>
        </motion.div>
